...
namespace LightBDD.SummaryGeneration.UnitTests
{
    ...
    public class FeatureSummaryExtensions_tests
    {
        ...
        public void It_should_count_scenarios()
        {
            var features = new[]
            {
                new Results.TestFeatureResult
                {
                    Scenarios = new[]
                    {
                        new Results.TestScenarioResult(),
                        new Results.TestScenarioResult()
                    }
                },
                new Results.TestFeatureResult
        ...
        public void It_should_count_steps()
        {
            var features = new[]
            {
                new Results.TestFeatureResult
                {
                    Scenarios = new[]
                    {
                        new Results.TestScenarioResult {Steps = new[] {new Results.TestStepResult()}},
                        new Results.TestScenarioResult {Steps = new[] {new Results.TestStepResult()}}
                    }
                },
                new Results.TestFeatureResult
            ...
            var features = new[]
            {
                new Results.TestFeatureResult
                {
                    Scenarios = new[]
                    {
                        new Results.TestScenarioResult
                        {
                            Steps = new[] {new Results.TestStepResult {Status = ExecutionStatus.Passed}}
                        },
                        new Results.TestScenarioResult
                        {
                            Steps = new[] {new Results.TestStepResult {Status = ExecutionStatus.Failed}}
                        }
                    }
                },
                new Results.TestFeatureResult
                {
                    Scenarios = new[]
                    {
                        new Results.TestScenarioResult
                        {
                            Steps = new[]
                            {
                                new Results.TestStepResult {Status = ExecutionStatus.Passed},
                                new Results.TestStepResult {Status = ExecutionStatus.Bypassed},
                                new Results.TestStepResult {Status = ExecutionStatus.Failed}
                            }
                        }
                    }
                }
        ...
        public void It_should_count_execution_times()
        {
            ...
            var features = new[]
            {
                ...
                new Results.TestFeatureResult
                {
                    Scenarios = new Results.TestScenarioResult[0]
                }
            };
