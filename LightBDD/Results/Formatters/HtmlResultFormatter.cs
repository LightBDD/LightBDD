...
namespace LightBDD.Results.Formatters
{
	...
	public class HtmlResultFormatter : IResultFormatter
	{
		...
		private void WriteHeader(HtmlTextWriter writer)
		{
			...
			WriteTag(writer, HtmlTextWriterTag.Style, null, _styles);
		...
		private static void WriteTag(HtmlTextWriter writer, HtmlTextWriterTag tag, string className, string content)
		{
			if (content == null)
				return;
			if (className != null)
