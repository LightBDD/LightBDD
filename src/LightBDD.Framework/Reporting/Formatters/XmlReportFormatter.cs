    ...
    public class XmlReportFormatter : IReportFormatter
    {
        ...
        public void Format(Stream stream, ITestRunResult result)
        {
            using (var writer = new StreamWriter(stream, new UTF8Encoding(false)))
                ToXDocument(result).Save(writer);
        }

        #endregion

        private static XDocument ToXDocument(ITestRunResult result)
        {
            return new XDocument(
                new XDeclaration("1.0", "utf-8", null),
                new XElement("TestResults",
                    Enumerable.Repeat(ToSummaryXElement(result), 1)
                        .Concat(result.Features.Select(ToXElement)).Cast<object>().ToArray()));
        }
        ...
        private static XElement ToXElement(IFeatureResult feature)
        {
            ...
            objects.Add(new XAttribute("RuntimeId", feature.Info.RuntimeId));
        ...
        private static XElement ToXElement(IScenarioResult scenario)
        {
            ...
            objects.Add(new XAttribute("RuntimeId", scenario.Info.RuntimeId));
        ...
        private static XElement ToXElement(IStepResult step, string elementName)
        {
            ...
            objects.Add(new XAttribute("RuntimeId", step.Info.RuntimeId));
