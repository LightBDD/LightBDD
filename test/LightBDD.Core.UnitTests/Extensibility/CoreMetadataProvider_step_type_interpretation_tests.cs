...
namespace LightBDD.Core.UnitTests.Extensibility
{
    ...
    public class CoreMetadataProvider_step_type_interpretation_tests
    {
        ...
        public void GetStepName_should_properly_convert_predefined_step_type(string inputStepType, string lastStepType, string expectedStepType)
        {
            ...
            var stepName = TestMetadataProvider.Default.GetStepName(descriptor, lastStepType);
        ...
        public void GetStepName_should_not_extract_step_type_if_nothing_is_left_after_it()
        {
            ...
            var stepName = TestMetadataProvider.Default.GetStepName(descriptor, null);
        ...
        {
            var metadataProvider = TestMetadataProvider.Create(cfg => cfg.ConfigureStepTypes().UpdateRepeatedStepReplacement(repeatedStepReplacement));

            var stepTypeName = "given";

            var descriptor = new StepDescriptor("some_name", (o, a) => Task.FromResult(DefaultStepResultDescriptor.Instance)) { PredefinedStepType = stepTypeName };
        ...
        {
            var metadataProvider = TestMetadataProvider.Create(cfg => cfg.ConfigureStepTypes().UpdatePredefinedStepTypes("call", "invoke"));
