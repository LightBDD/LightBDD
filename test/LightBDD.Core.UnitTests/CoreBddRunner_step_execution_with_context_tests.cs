    ...
    public class CoreBddRunner_step_execution_with_context_tests
    {
        ...
        public void Runner_should_instantiate_context_and_use_it_in_step_and_parameter_execution()
        {
            ...
            _runner.Test()
                ...
                .TestScenario(
                    new StepDescriptor("test1", (ctx, args) => { list.Add(ctx); return Task.FromResult(StepResultDescriptor.Default); }, ParameterDescriptor.FromInvocation(someParameterInfo, ctx => { list.Add(ctx); return ctx; })),
                    new StepDescriptor("test1", (ctx, args) => { list.Add(ctx); return Task.FromResult(StepResultDescriptor.Default); }, ParameterDescriptor.FromInvocation(someParameterInfo, ctx => { list.Add(ctx); return ctx; })));
        ...
        public void Runner_should_instantiate_context_for_every_scenario()
        {
            ...
            runnerWithContext.TestScenario(
                    new StepDescriptor("test1", (ctx, args) => { list.Add(ctx); return Task.FromResult(StepResultDescriptor.Default); }),
                    new StepDescriptor("test1", (ctx, args) => { list.Add(ctx); return Task.FromResult(StepResultDescriptor.Default); }));
