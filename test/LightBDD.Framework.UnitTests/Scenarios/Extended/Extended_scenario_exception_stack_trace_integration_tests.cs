...
namespace LightBDD.Framework.UnitTests.Scenarios.Extended
{
    ...
    public class Extended_scenario_exception_stack_trace_integration_tests
    {
        ...
        {
            var scenario = await TestableBddRunner.Default.RunScenario(r => r.RunScenarioAsync(_ => Step_throwing_exception()));
...
\s*at [^\n]*lambda_method[^\n]*
([^\n]*
)?\s*at LightBDD.Framework.Scenarios.ExtendedExtensions.RunScenario[^\n]*");
        }
        ...
        {
            var scenario = await TestableBddRunner.Default.RunScenario(r => r.RunScenarioAsync(_ => Async_step_throwing_exception_immediately()));
            var ex = scenario.ExecutionException.ShouldBeOfType<InvalidOperationException>();
            ex.AssertStackTraceMatching(
...
([^\n]*
)?\s*at LightBDD.Framework.Scenarios.ExtendedExtensions[^\n]*RunScenarioAsync[^\n]*");
        }
        ...
        {
            var scenario = await TestableBddRunner.Default.RunScenario(r => r.RunScenarioAsync(_ => Async_step_throwing_exception_after_await()));
            var ex = scenario.ExecutionException.ShouldBeOfType<InvalidOperationException>();
            ex.AssertStackTraceMatching(
