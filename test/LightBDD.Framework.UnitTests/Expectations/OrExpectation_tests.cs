...
namespace LightBDD.Framework.UnitTests.Expectations
{
    ...
    public class OrExpectation_tests
    {
        ...
        {
            var expectation = Expect.To.Empty().Or(x => x.Null()).Or(x => x.Contains('c'));
        ...
        {
            Assert.That(Expect.To.GreaterThan(3).Or(x => x.Not.LessThan(5)).ToString(), Is.EqualTo("(greater than '3' or not less than '5')"));
        }
    }
}