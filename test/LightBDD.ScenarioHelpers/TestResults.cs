...
namespace LightBDD.ScenarioHelpers
{
    public static class TestResults
    {
        ...
        {
            private static readonly AssemblyInfo[] Versions = new[] { typeof(ITestRunInfo).Assembly }.Select(AssemblyInfo.From).ToArray();
            ...
            public IReadOnlyList<AssemblyInfo> LightBddAssemblies { get; set; } = Versions;
        }
        ...
        public static TestTestRunResult CreateTestRunResults(params TestFeatureResult[] features)
        {
            var summary = features.SelectMany(f => f.GetScenarios()).Select(s => s.ExecutionTime)
                .Aggregate((start: DateTimeOffset.MaxValue, end: DateTimeOffset.MinValue),
                    (c, e) => (
                        e.Start < c.start ? e.Start : c.start,
                        e.End > c.end ? e.End : c.end));

            return new TestTestRunResult
            {
                Features = features,
                ExecutionTime = new TestExecutionTime { Start = summary.start, Duration = summary.end - summary.start },
